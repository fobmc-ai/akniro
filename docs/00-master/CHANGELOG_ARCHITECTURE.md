...
- AI Apply 进一步强制绑定同项目 VALIDATED 测试证据，未测试建议不能落地。
- Edge 同步增加只读重放安全摘要，统一展示幂等、PUSH 审批、冲突和失败状态，并接入 Ops 页面。
- Ops 页面增加带理由的人工同步冲突裁决入口，可受控重新排队或确认失败。
- 增加 Deployment Preflight，按授权、暂存、观察阶段展示部署请求是否满足条件，不执行真实部署。
...
- 技术债审计将 TD-001 从“Runtime 未实现”修正为“软件软运行时已缓解、现场实时认证仍开放”，与 PLC-002 模拟接口和证据现状保持一致。
